...
Cross-Site Request Forgery (CSRF) attacks could be mitigated by implementing Cross-Origin Resource Sharing (CORS) headers. For security reasons, the 'root' username and password was updated within the database before going live to allow for a private administrator username and password. Users who want to demo the web application can now log in with username 'administrator' and password 'root'.
